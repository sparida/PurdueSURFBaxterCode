...
def main():
	...
	rospy.init_node('CamToolTableCalib', disable_signals=True)
	...
	cam_arm_z = ((baxter_endpoints[0].z + baxter_endpoints[2].z) / 2) + 0.2	 # Height above table
	...
	bcajpFile = open("BaxterCamArmJointPosFile.dat", "w");
	...
	ImageClipPosFile = open("ImageClipPosFile.dat", "w");
	...
	XAPosFile = open("XAPosFile.dat", "w");
	...
	if(numpy.linalg.det(X) == 0):
		...
		tmatFile = open("TransformationMatrixFile.dat", "w");
